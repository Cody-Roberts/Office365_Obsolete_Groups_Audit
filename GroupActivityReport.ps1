...
if(! (Test-Path -Path "$ProgramHomeDir")){
   Set-Location -Path "$ProgramHomeDir"
}

##Sends input to Connect0365Services which connects to Office 365 Modules
##Edit which tenant to use after SharePointHostName
.\ConnectO365Services.ps1 -Services AzureAD, ExchangeOnline, SharePoint, Teams -SharePointHostName *Tenant_Name_Here* -UserName $UserName -Password $SecureString
